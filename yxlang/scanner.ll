...
"if"         { BEGIN IFSTATE; return token::IF; }
"then"       { return token::THEN; }
"else"       { return token::ELSE; }
"fi"         { BEGIN INITIAL; return token::FI; }
...
[A-Za-z][A-Za-z0-9]*"_"[A-Za-z][A-Za-z0-9]* { yylval->stringVal = new std::string(yytext, yyleng); return token::METHOD; }
 /* [A-Za-z][A-Za-z0-9_,.-]* { yylval->stringVal = new std::string(yytext, yyleng); return token::STRING; } */
[A-Za-z][A-Za-z0-9_.-]* { yylval->stringVal = new std::string(yytext, yyleng); return token::STRING; }
[\"][A-Za-z][A-Za-z0-9_.-]*[\"] { yylval->stringVal = new std::string(yytext + 1, yyleng - 2); return token::LITERAL; }
...
\r { yylloc->lines(yyleng); yylloc->step(); return token::EOL; }
